...
// 新版本命名、宿主→成员下载回程，以及大文件(100MB)流式不卡死。
...
;(async () => {
  ...
  host.on('done', (info) => {
    if (info.share && info.share.op === 'upload') {
      const r = store.placeUpload({ fileName: info.fname, tempPath: info.tempPath, uploadedBy: info.from, intent: info.share.intent, entryId: info.share.entryId, parentId: info.share.parentId })
      results.push({ r })
    }
  })
  ...
  member.sendFile('B', f1, 'share', 'm1', 'sp', null, false, 'm1', Date.now(), { op: 'upload', spaceId: 'sp', parentId: ss.ROOT_ID, intent: 'new' })
  ...
  assert.strictEqual(results[0].r.version.fileHash, h1, '宿主端 SHA-256 == 源(端到端校验)')
  assert.ok(fs.existsSync(path.join(root, 'files', '需求文档.docx')), '宿主物理文件存在')

  const f2 = path.join(srcDir, 'edited.docx')
  fs.writeFileSync(f2, Buffer.from('v1-' + 'y'.repeat(8000)))
  member.sendFile('B', f2, 'share', 'm2', 'sp', null, false, 'm2', Date.now(), { op: 'upload', spaceId: 'sp', parentId: ss.ROOT_ID, intent: 'version', entryId })
  for (let i = 0; i < 60 && results.length < 2; i++) await wait(100)
  assert.strictEqual(results.length, 2, '宿主应收到第2次上传')
  assert.strictEqual(results[1].r.version.versionFileName, '需求文档_V01.docx', '新版本命名 _V01')

  let dl = null
  member.on('done', (info) => { if (info.share && info.share.op === 'download') dl = info })
  const vp = store.versionAbsPath(entryId, null)
  peers.A.tport = member.tport
  host.sendFile('A', vp.abs, 'share', 'd1', 'sp', null, false, 'd1', Date.now(), { op: 'download', spaceId: 'sp', entryId })
  for (let i = 0; i < 60 && !dl; i++) await wait(100)
  assert.ok(dl, '成员应收到下载 done')
  const dlBuf = fs.readFileSync(dl.tempPath)
  assert.strictEqual(crypto.createHash('sha256').update(dlBuf).digest('hex'), vp.hash, '下载内容 SHA-256 == 宿主记录')
  ...
  member.sendFile('B', big, 'share', 'mbig', 'sp', null, false, 'mbig', Date.now(), { op: 'upload', spaceId: 'sp', parentId: ss.ROOT_ID, intent: 'new' })
  for (let i = 0; i < 300 && results.length < before + 1; i++) await wait(100)
  assert.strictEqual(results.length, before + 1, '大文件应上传成功')
  assert.strictEqual(results[before].r.version.fileHash, bigHash, '100MB 文件 SHA-256 一致')

  host.stop(); member.stop()
  try { fs.rmSync(root, { recursive: true, force: true }); fs.rmSync(srcDir, { recursive: true, force: true }) } catch (_) {}
  console.log('✅ 共享空间端到端传输（上传/新版本/下载/100MB + 端到端 SHA-256）验证通过')
  process.exit(0)
